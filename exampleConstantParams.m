...
function propagateTruthTrajectory()
    ...
    propTime = 30;
    [trueTimeHistory, trueStateHistory, measHistory] = propagator.propagateWithSensors(propTime, { rangeSensor, directionSensor, accelerometerSensor, rollGyroSensor });
...
function output = runEstimator(option)
    ...
    switch option
        case "batch"
            estimator = BatchEstimator(projectileModelDynamics, { rangeSensorModel, directionSensorModel, accelerometerSensorModel, rollGyroSensorModel });
        case "sequential"
            estimator = SequentialEstimator(projectileModelDynamics, { rangeSensorModel, directionSensorModel, accelerometerSensorModel, rollGyroSensorModel });
        case "sqrtsequential"
            estimator = SqrtSequentialEstimator(projectileModelDynamics, { rangeSensorModel, directionSensorModel, accelerometerSensorModel, rollGyroSensorModel });
        otherwise
...
function plotResults()
    ...
    gyroHistory = measHistory(2:end, measHistory(1, :) == 4);
    ...
    priorGyroResiduals = priorMeasResiduals(2:end, priorMeasResiduals(1, :) == 4);
    ...
    postGyroResiduals = postMeasResiduals(2:end, postMeasResiduals(1, :) == 4);
    
    % Get convergence histories
    CDIterations = output.iterations.params(1, :);
    CDStdDevIterations = output.iterations.paramCovar(1, :) .^ 0.5;
    CDPlusIterations = CDIterations + CDStdDevIterations;
    CDMinusIterations = CDIterations - CDStdDevIterations;

    vWindxIterations = output.iterations.params(2, :);
    vWindxStdDevIterations = output.iterations.paramCovar(5, :) .^ 0.5;
    ...
    vWindyIterations = output.iterations.params(3, :);
    vWindyStdDevIterations = output.iterations.paramCovar(9, :) .^ 0.5;
    ...
    subplot(1, 3, 1)
    patch([0:nIterations, flip(0:nIterations)], [CDPlusIterations, flip(CDMinusIterations)], [1, 0.8, 0.8], "EdgeColor", "none")
    hold on
    plot(0:nIterations, CDIterations, 'rx-', "LineWidth", 1.5)
    hold off
    box on
    xlim([1, nIterations])
    xlabel("Iterations")
    ...
    subplot(1, 3, 2)
    patch([0:nIterations, flip(0:nIterations)], [vWindxPlusIterations, flip(vWindxMinusIterations)], [1, 0.8, 0.8], "EdgeColor", "none")
    hold on
    plot(0:nIterations, vWindxIterations, 'rx-', "LineWidth", 1.5)
    hold off
    box on
    xlim([1, nIterations])
    xlabel("Iterations")
    ...
    subplot(1, 3, 3)
    patch([0:nIterations, flip(0:nIterations)], [vWindyPlusIterations, flip(vWindyMinusIterations)], [1, 0.8, 0.8], "EdgeColor", "none")
    hold on
    plot(0:nIterations, vWindyIterations, 'rx-', "LineWidth", 1.5)
    hold off
    box on
    xlim([1, nIterations])
    xlabel("Iterations")
