...
function propagateTruthTrajectory()
    ...
    projectile.stateDef.state = [0; 0; 0; 28.7614; 0; -328.7443; 3.4558E+03];
...
function output = runEstimator(option)
    ...
    projectileModel.stateDef.state = [0; 0; 0; 28.7614; 0; -328.7443; 3.4558E+03];
    % projectileModel.stateDef.covar = diag([0.01; 0.01; 0.01; 0.5; 0.5; 5; 6.2832] .^ 2);  % TODO: Translate (V, az, el) with covars to (vx, vy, vz)
    projectileModel.stateDef.covar = diag([0; 0; 0; 1; 1; 10; 10.4720 * 10] .^ 2);  % TODO: Translate (V, az, el) with covars to (vx, vy, vz)
